...
// Driver wraps a keyvind.Matcher for use inside a bubbletea v2 Update loop. It
// feeds converted key-press messages to the matcher and arms / resolves the
// ambiguity timeout via tea.Tick. It is not safe for concurrent use; drive it
...
// Update processes one bubbletea message. For a key-press message it feeds every
// decoded key to the matcher; for the internal timeout message it resolves a
// held ambiguous match (ignoring stale timers). It returns the commands the host
// should dispatch, plus an optional tea.Cmd that the host MUST return from its
// own Update so the timeout fires. Other messages yield (nil, nil).
func (d *Driver) Update(msg tea.Msg) ([]keyvind.Command, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.KeyPressMsg:
		var cmds []keyvind.Command
		var res keyvind.Result
		for _, k := range FromKeyPress(msg) {
